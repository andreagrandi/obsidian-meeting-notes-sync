...
function findCandidates(
	...
	const scored: ScoredCandidate[] = [];
		...
		const overlapFraction = overlapMs / shorterDurationMs;
		if (overlapFraction < threshold || overlapMs < minOverlapMs) {
			continue;
		}
		const titleSimilarity = normalizedTitleSimilarity(meeting.title, recordTitle(record));
		const score = scoreCandidate(overlapFraction, titleSimilarity);
		scored.push({ recordKey, record, overlapMs, overlapFraction, titleSimilarity, score });
	}

	return scored.sort((a, b) => b.score - a.score);
}

/** Score a candidate: overlap is dominant; title similarity breaks ties. */
function scoreCandidate(overlapFraction: number, titleSimilarity: number): number {
	return overlapFraction * 100 + titleSimilarity;
}
